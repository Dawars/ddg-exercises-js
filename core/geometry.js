...
class Geometry {
    ...
    angle(c) {
        ...
        let angle = Math.acos(dot / (edge1.length() * edge2.length()));
    ...
    cotan(h) {
        ...
        let cos = dot / (edge1.norm() * edge2.norm());  // dot product
        let sin = cross / (edge1.norm() * edge2.norm());  // cross product
